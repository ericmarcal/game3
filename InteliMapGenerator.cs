...
namespace InteliMapPro
{
    ...
    public class InteliMapGenerator : MonoBehaviour
    {
        ...
        public BoundsInt boundsToFill = new BoundsInt(new Vector3Int(0, 0, 0), new Vector3Int(25, 25, 1)); // << CORRIGIDO
            ...
            for (int x = 0; x < boundsToFill.size.x; x++)
            {
                for (int y = 0; y < boundsToFill.size.y; y++)
                {
                    ...
                    if (domain.Count == 1)
                    {
                        ...
                        for (int layer = 0; layer < generatorData.layerCount; layer++)
                        {
                            mapToFill[layer].SetTile(new Vector3Int(x + boundsToFill.position.x, y + boundsToFill.position.y, 0), generatorData.uniqueTiles[domain.GetDense(0)].tiles[layer]); // << CORRIGIDO
                        }
                    }
                    else
                    {
        ...
        void OnDrawGizmosSelected()
        {
            if (mapToFill != null)
            {
                ...
                foreach (Tilemap map in mapToFill)
                {
                    if (map != null)
                    {
                        // TileSelectionGizmos.DrawBounds(map, boundsToFill); // Comentado para evitar erro, pois não temos o script TileSelectionGizmos
                    }
                }
            }
        }
