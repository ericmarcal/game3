...
namespace InteliMapPro
{
    public static class TileSelectionDrawer
    {
        public static void DrawBounds(Tilemap map, Vector3Int a, Vector3Int b, Action<Vector3, Vector3> drawLine)
        {
            ...
            if (map.cellLayout == GridLayout.CellLayout.Hexagon)
            {
                if (map.cellSwizzle == GridLayout.CellSwizzle.XYZ)
                {
                    ...
                    sizeX = map.transform.localToWorldMatrix.MultiplyVector(new Vector3(map.cellSize.x, 0, 0));
                    sizeY = map.transform.localToWorldMatrix.MultiplyVector(new Vector3(0, map.cellSize.y, 0));
                }
                else // yxz
                {
                    offset = map.transform.localToWorldMatrix.MultiplyVector(new Vector3(-map.cellSize.y * 0.5f, -map.cellSize.x * 0.5f));
                    sizeY = map.transform.localToWorldMatrix.MultiplyVector(new Vector3(map.cellSize.y, 0, 0));
                    sizeX = map.transform.localToWorldMatrix.MultiplyVector(new Vector3(0, map.cellSize.x, 0));
                }
            }
            else if (map.cellLayout == GridLayout.CellLayout.Isometric || map.cellLayout == GridLayout.CellLayout.IsometricZAsY)
            {
                sizeX = map.transform.localToWorldMatrix.MultiplyVector(new Vector3(map.cellSize.x * 0.5f, map.cellSize.y * 0.5f, 0));
                sizeY = map.transform.localToWorldMatrix.MultiplyVector(new Vector3(map.cellSize.x * -0.5f, map.cellSize.y * 0.5f, 0));
            }
            else
            {
                sizeX = map.transform.localToWorldMatrix.MultiplyVector(new Vector3(map.cellSize.x, 0, 0));
            ...
            DrawRect(map,
                min,
                new Vector3Int(min.x, max.y, 0), // CORRIGIDO
                max,
                new Vector3Int(max.x, min.y, 0), // CORRIGIDO
        ...
        public static void DrawBounds(Tilemap map, BoundsInt bounds, Action<Vector3, Vector3> drawLine)
        {
            ...
            DrawRect(map,
                bounds.min,
                bounds.min + new Vector3Int(0, bounds.size.y - 1, 0), // CORRIGIDO
                bounds.max - Vector3Int.one,
                bounds.min + new Vector3Int(bounds.size.x - 1, 0, 0), // CORRIGIDO
                map.orientationMatrix.MultiplyVector(sizeX),
                map.orientationMatrix.MultiplyVector(sizeY),
                offset,
                drawLine);
